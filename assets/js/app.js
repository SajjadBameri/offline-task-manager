...
setTimeout(() => { if (login) login.classList.remove('dis-hide'); }, 4000);
...
if (send) {
    send.addEventListener('click', function (e) {
        ...
        if (floatingInput !== '' && floatingInput.length > 3)
...
function toggleCompletedTasks() {
    ...
    if (switchCheckDefault) {
        if (showOnlyCompleted) {
            ...
            switchCheckDefault.innerHTML = '<i class="bi bi-check-circle-fill"></i> نمایش همه';
        } else {
            switchCheckDefault.classList.remove('active', 'btn-success');
            switchCheckDefault.classList.add('btn-outline-secondary');
            switchCheckDefault.innerHTML = '<i class="bi bi-check-circle"></i> فقط انجام شده‌ها';
        }
    }
...
function displayData() {
    ...
    request.onsuccess = (e) => {
        ...
        if (cursor) {
            ...
            const createdAt = cursor.value.createdAt ? new Date(cursor.value.createdAt) : new Date();
            ...
            task.className = 'task border rounded-4 p-4  mb-3 bg-transparent shadow-sm';
            task.setAttribute('data-id', taskId);

            const titleEl = document.createElement('h5');
            titleEl.className = 'mb-2 fw-bold ';

            if (currentSearchTerm !== '' && title.toLowerCase().includes(currentSearchTerm)) {
                const regex = new RegExp(`(${currentSearchTerm})`, 'gi');
                const highlightedTitle = title.replace(regex, '<span class="bg-warning  px-1 rounded">$1</span>');
            ...
            if (currentSearchTerm !== '' && discription.toLowerCase().includes(currentSearchTerm)) {
                const regex = new RegExp(`(${currentSearchTerm})`, 'gi');
                const highlightedDesc = discription.replace(regex, '<span class="bg-warning  px-1 rounded">$1</span>');
            ...
            doneText.className = 'fs-6 ' + (completed ? 'text-success' : 'text-muted');
...
function showNoCompletedTasksMessage() {
    ...
    msg.innerHTML = `
        ...
        <p class="lead">هنوز هیچ تسکی را به عنوان "انجام شده" علامت نزده‌اید</p>
        <button id="showAllTasks" class="btn btn-outline-primary mt-3">نمایش همه تسک‌ها</button>
    `;
    from.appendChild(msg);
...
function showCompletedTasksMessage(count) {
    ...
    msg.innerHTML = `
        ...
        در حال نمایش ${count} تسک انجام شده
...
function updateCount(hasResults = true, completedCount = 0, totalCount = 0) {
    setTimeout(() => {
        ...
        if (countEl) {
            if (showOnlyCompleted) {
                countEl.textContent = `${completedCount} تسک انجام شده`;
